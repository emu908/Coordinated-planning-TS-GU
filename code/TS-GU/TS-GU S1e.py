...
def gurobi(sol):
    ...
    # 场景1.5直接固定x的值
    for d in d_range:
        model.addConstr(quicksum(x[d, 1, 2, n] for n in n_range) == 1, name=f"extra_x_1_{d}")
        model.addConstr(quicksum(x[d, 2, 10, n] for n in n_range) == 1, name=f"extra_x_2_{d}")
        model.addConstr(quicksum(x[d, 3, 18, n] for n in n_range) == 1, name=f"extra_x_3_{d}")
        model.addConstr(quicksum(x[d, 4, 25, n] for n in n_range) == 1, name=f"extra_x_4_{d}")
        model.addConstr(quicksum(x[d, 5, 32, n] for n in n_range) == 1, name=f"extra_x_5_{d}")
        model.addConstr(quicksum(x[d, 6, 39, n] for n in n_range) == 1, name=f"extra_x_6_{d}")
        model.addConstr(quicksum(x[d, 7, 47, n] for n in n_range) == 1, name=f"extra_x_7_{d}")
        model.addConstr(quicksum(x[d, 8, 55, n] for n in n_range) == 1, name=f"extra_x_8_{d}")
