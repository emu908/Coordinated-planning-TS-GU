...
def evaluate(sol):
                ...
                if k == 1:  # 首先生成行程1的情况
                    if s == 1:
                        ...
                        for k_1 in k_bus_range:
                            if 1 <= A_bus[k_1, 2]+t_walk <= D_art[d, k, s]:
                    ...
                    if s == 1:
                        ...
                        for k_1 in k_bus_range:
                            if D_art[d, k-1, s]+1 <= A_bus[k_1, 2]+t_walk <= D_art[d, k, s]:
                                # 如果在第k趟art出发前有公交到达，则下车乘客计入art候车乘客序列
        ...
        if k == 1:
            ...
            for d in d_range:
                for k_1 in k_art_range:
                    if 1 <= A_art[d, k_1, s_d_trans[d - 1]]+t_walk <= D_bus[k, 2]:
            ...
            for d in d_range:
                for k_1 in k_art_range:
                    if D_bus[k-1, 2]+1 <= A_art[d, k_1, s_d_trans[d - 1]]+t_walk <= D_bus[k, 2]:
                        section = math.floor((A_art[d, k_1, s_d_trans[d - 1]] - 1) / 10)
                        jt += r_a[d, k_1, s_d_trans[d - 1]] * proportion_a_to_b[section]
    ...
    Z1_2 += t_walk * sum(r_a[d, k, s_d_trans[d - 1]] * proportion_a_to_b[math.floor((A_art[d, k, s_d_trans[d - 1]] - 1-t_walk) / 10)] for d in d_range for k in k_art_range)
    ...
    for d in d_range:
        for k in k_art_range:
            ...
            Z2_2 += t_walk * r_a[d, k, s_d_trans[d - 1]] * proportion_a_to_b[section]
...
def gurobi(sol):
    ...
    e_bus = model.addVar(lb=1, ub=5, vtype=GRB.INTEGER, name="e_bus")
    ...
    for d in d_range:
        for k in k_art_range:
            for t in t_1_range:
                ...
                model.addConstr(nu[d, k, t] <= j_a[t-t_s_bus[0]-t_walk, 2], name=f"constraint_70_2_{d}_{k}_{t}")
                model.addConstr(nu[d, k, t] >= j_a[t-t_s_bus[0]-t_walk, 2] - M * (1 - m_union[d, k, s_d_trans[d-1], t]), name=f"constraint_70_3_{d}_{k}_{t}")
    ...
    for d in d_range:
        for k in k_art_range:
            for t in t_range:
                model.addConstr(omg[d, k, t] <= M * gamma[d, k, t+t_s_bus[0]-t_walk], name=f"constraint_73_1_{d}_{k}_{t}")
                model.addConstr(omg[d, k, t] <= r_a[d, k, s_d_trans[d-1]], name=f"constraint_73_2_{d}_{k}_{t}")
                model.addConstr(omg[d, k, t] >= r_a[d, k, s_d_trans[d-1]] - M * (1 - gamma[d, k, t+t_s_bus[0]-t_walk]), name=f"constraint_73_3_{d}_{k}_{t}")
                model.addConstr(omg[d, k, t] >= 0, name=f"constraint_73_4_{d}_{k}_{t}")

    # 约束74（线性化过程）
    for t in t_range:
        if t != 1:
            section = math.floor((t+t_s_bus[0]-t_walk-1) / 10)
    ...
    rhs_52_2 = a_2 * quicksum(j_w[t, 2] for t in t_range) + a_2 * t_walk * quicksum(omg[d, k, t] * proportion_a_to_b[math.floor((t+t_s_bus[0]-t_walk-1) / 10)] for d in d_range for k in k_art_range for t in t_range)
...
if __name__ == "__main__":
    ...
    # 求解模型
    model.optimize()
    model.write("/Users/hatsuyuki/Downloads/518.sol")
    if model.status == GRB.INFEASIBLE:
        model.computeIIS()
        model.write("/Users/hatsuyuki/Downloads/4.22.ilp")
