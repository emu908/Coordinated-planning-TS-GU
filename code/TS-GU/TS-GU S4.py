...
def evaluate(sol):
    ...
    Z2_1 = 20 * sum(arrival_rate_bus[t] for t in t_range)
    Z2_2 = 0
    for d in d_range:
        for k in k_art_range:
            section = math.floor((A_art[d, k, s_d_trans[d - 1]] - 1) / 10)
            Z2_2 += r_a[d, k, s_d_trans[d - 1]] * proportion_a_to_b[section] * 20
...
def get_neighbors(sol, N=40):
    ...
    for _ in range(N):
        ...
        # 随机在一个 (d,k) 上换发车时刻或编组
        if random.random() < 5:  # 随机生成0-1的浮点数
            # 选择改变ART的行程
...
def gurobi(sol):
    ...
    j_w = model.addVars(t_range, [2], lb=0, vtype=GRB.CONTINUOUS, name="j_w")
    j_a = model.addVars(range(21, 81), [2], lb=0, vtype=GRB.CONTINUOUS, name="j_a")
    ...
    omg = model.addVars(d_range, k_art_range, t_m_range, lb=0, vtype=GRB.CONTINUOUS, name="omg")
    ...
    # 约束32
    for t in range(21, 81):
        model.addConstr(j_a[t, 2] == arrival_rate_bus[t - 20], name=f"constraint_32_{t}")

    # 约束34
    for d in d_range:
        for k in k_art_range:
            model.addConstr(rt_w[d, k] == quicksum(
                m_union[d, k, s_d_trans[d - 1], t] * n_d_trans[d - 1] * j_a[t, 2] for t in range(21, 81)),
                            name=f"constraint_34_{d}_{k}")
    ...
    for d in d_range:
        for k in k_art_range:
            if k == 1:
                rhs_35 = quicksum(
                    m_union[d, k, s_d_trans[d - 1], t] * arrival_rate[d, s_d_trans[d - 1], t] for t in t_m_range) + \
                         rt_w[d, k]
                model.addConstr(r_w[d, k, s_d_trans[d - 1]] == rhs_35, name=f"constraint_35_{d}_{k}")
            else:
                rhs_35 = r_w[d, k - 1, s_d_trans[d - 1]] - r_b[d, k - 1, s_d_trans[d - 1]] + quicksum(
                    m_union[d, k, s_d_trans[d - 1], t] * arrival_rate[d, s_d_trans[d - 1], t] for t in t_m_range) + \
                         rt_w[d, k]
                model.addConstr(r_w[d, k, s_d_trans[d - 1]] == rhs_35, name=f"constraint_35_{d}_{k}")
    ...
    for d in d_range:
        for k in k_art_range:
            for t in t_range:
                model.addConstr(omg[d, k, t] <= M * gamma[d, k, t], name=f"constraint_73_1_{d}_{k}_{t}")
                model.addConstr(omg[d, k, t] <= r_a[d, k, s_d_trans[d - 1]], name=f"constraint_73_2_{d}_{k}_{t}")
                model.addConstr(omg[d, k, t] >= r_a[d, k, s_d_trans[d - 1]] - M * (1 - gamma[d, k, t]),
                                name=f"constraint_73_3_{d}_{k}_{t}")
                model.addConstr(omg[d, k, t] >= 0, name=f"constraint_73_4_{d}_{k}_{t}")

    # 约束71（38的转化）
    for t in t_range:
        section = math.floor((t - 1) / 10)
        model.addConstr(
            j_w[t, 2] == quicksum(omg[d, k, t] * proportion_a_to_b[section] for d in d_range for k in k_art_range),
            name=f"constraint_71_{t}")
            ...
            for s in s_art_range:
                for t in t_m_range:
                    if s != s_d_trans[d - 1]:
                        ...
                        model.addConstr(rou[d, k, s, t] <= M * (1 - m[d, k, s, t]),
                                        name=f"constraint_77_2_{d}_{k}_{s}_{t}")
                        model.addConstr(rou[d, k, s, t] >= r_b[d, k, s] - M * m[d, k, s, t],
                                        name=f"constraint_77_3_{d}_{k}_{s}_{t}")
    ...
    for d in d_range:
        for k in k_art_range:
            for t in t_m_range:
                ...
                model.addConstr(rou_1[d, k, t] <= M * (1 - m[d, k, s_d_trans[d - 1], t]),
                                name=f"constraint_79_2_{d}_{k}_{t}")
                model.addConstr(rou_1[d, k, t] >= r_b[d, k, s_d_trans[d - 1]] - M * m[d, k, s_d_trans[d - 1], t],
                                name=f"constraint_79_3_{d}_{k}_{t}")
                model.addConstr(rou_1[d, k, t] >= 0, name=f"constraint_79_4_{d}_{k}_{t}")

    # 目标函数75（51的转化）
    a_1 = 1
    a_2 = 2
    rhs_75_1 = a_1 * quicksum((quicksum(arrival_rate[d, s, t] * (t_m_range[-1] - t) for t in t_m_range) - quicksum(
        rou[d, k, s, t] for k in k_art_range for t in t_m_range)) for d in d_range for s in s_art_range if
                              s != s_d_trans[d - 1])
    rhs_75_2 = a_2 * quicksum((quicksum(
        arrival_rate[d, s_d_trans[d - 1], t] * (t_m_range[-1] - t) for t in t_m_range) - quicksum(
        rou_1[d, k, t] for k in k_art_range for t in t_m_range)) for d in d_range) + a_2 * quicksum(
        j_a[t, 2] * (t_m_range[-1] - t) for t in range(21, 81))
    ...
    # 目标函数52
    rhs_52_1 = a_1 * 20 * quicksum(j_a[t, 2] for t in range(21, 81))
    rhs_52_2 = a_2 * 20 * quicksum(j_w[t, 2] for t in t_range)
    ...
    pc_art = 500  # art单元的采购成本
    tao = 10
    rhs_53 = pc_art * quicksum(c_g[g] for g in g_range) + tao * (quicksum(
        t_art_total * n * x[d, k, t, n] for d in d_range for k in k_art_range for t in t_range for n in n_range) +
                                                                 quicksum(
                                                                     t_depot_in * n * p_in[t, g, n] for g in g_range for
                                                                     t in T_art_depot_in for n in n_range) +
                                                                 quicksum(
                                                                     t_depot_out * n * p_out[t, g, n] for g in g_range
                                                                     for t in T_art_depot_out for n in n_range) +
                                                                 quicksum(
                                                                     t_turn_in * n * q_in[t, g, n] for g in g_range for
                                                                     t in T_art_turn_in for n in n_range) +
                                                                 quicksum(
                                                                     t_turn_out * n * q_out[t, g, n] for g in g_range
                                                                     for t in T_art_turn_out for n in n_range))
